...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST['markMembership']) && isset($_POST['selectedMembers']) && is_array($_POST['selectedMembers']) && isset($_POST['durationDays'])) {
            ...
            foreach ($selectedMembers as $memberID) {
                ...
                $membershipStatus = ($durationDays > 0) ? 'Active' : 'Expired';
    ...
    $addedMembersSQL = "SELECT member.MemberID, member.FirstName, member.LastName, morning_shift.MembershipDurationDays FROM member INNER JOIN morning_shift ON member.MemberID = morning_shift.MemberID";
            ...
            if ($removeMembersResult && $removeMembersResult->num_rows > 0) {
                while ($row = $removeMembersResult->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>{$row['FirstName']}</td>
                            <td>{$row['LastName']}</td>
                            <td><input type='number' name='durationDays' value='" . ($row['MembershipDurationDays'] > 0 ? $row['MembershipDurationDays'] : 0) . "'></td>
            ...
            if ($addedMembersResult && $addedMembersResult->num_rows > 0) {
                while ($row = $addedMembersResult->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>" . ($row['MembershipDurationDays'] > 0 ? 'Active' : 'Expired') . "</td>
